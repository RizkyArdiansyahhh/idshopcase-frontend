...
import VariantField from "./_components/variant-field";
...
const CreateProductPage = () => {
  ...
  // .refine(
  //   (data) => {
  //     if (data.toggleIsVariant) {
  //       return data.variantOptions && data.variantOptions.length > 0;
  //     }
  //     return true;
  //   },
  //   {
  //     message: "Variasi tidak boleh kosong",
  //     path: ["variantOptions"],
  //   }
  // );
  ...
  const isNextEnabled =
    ...
    variantOptions.every((variant) => {
      const isValidVariant =
        variant.valueVariant &&
        variant.valueVariant.length > 0 &&
        variant.valueVariant.every((value) => value.label.trim() !== "");
      return isValidVariant && variant.nameVariant.trim() !== "";
    });

  const handleSubmit = (data: CreateProductType) => {
    console.log(data);
              ...
              <div className="h-full flex flex-col gap-2">
                ...
                <div
                  className={`flex  justify-center py-2 ${
                    (!fieldsVariantOptions.length || !isVariant) &&
                    "h-full items-center"
                  }`}
                >
                  <Button
